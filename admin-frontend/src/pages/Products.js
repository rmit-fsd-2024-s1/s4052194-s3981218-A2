...
const Products = () => {
  ...
  const handleCreateProduct = async () => {
    const updatedProduct = {
      ...newProduct,
      product_image: "/null.png",
    };
    const data = await createProduct(updatedProduct);
  ...
  const handleUpdateProduct = async (product_id, updatedProduct) => {
    ...
    setProducts(
      products.map((product) =>
        product.product_id === product_id ? data : product
      )
    );
  ...
  const handleToggleSpecialProduct = async (product_id, is_special) => {
    const data = is_special
      ? await unmarkSpecialProduct(product_id)
      : await markSpecialProduct(product_id);
    setProducts(
      products.map((product) =>
                ...
                <>
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) =>
                        (product.product_price = parseFloat(e.target.value))
                      }
                    />
                  </td>
                  <td>
                    <input
                      ...
                      onChange={(e) => (product.product_image = e.target.value)}
                    />
                  </td>
                  <td>
                    <input
                  ...
                  <td>
                    <button
                      onClick={() =>
                        handleUpdateProduct(product.product_id, product)
                      }
                    >
                      Save
                    </button>
